...
Using the tagger
----------------
Installing the piboso module will also install the command `piboso`, and
this can be used to invoke a CLI interface to the tagger. The simplest 
invocation is

    piboso -o <OUTPUT_PATH> <FILE TO TAG> <FILE TO TAG> ...

If no files are specified on the command line, `piboso` will read STDIN
and interpret each line as a path to a file to be tagged. More detailed 
information about invoking `piboso` can be obtained by invoking
...
* [4] http://nltk.org
* [5] http://www.ims.uni-stuttgart.de/forschung/ressourcen/werkzeuge/treetagger.html
